...
!       end select
  ...
  case("LIMIT")
    ...
    if (item < nitems) then
      call reada(aa)
      if (upcase(aa) == "ALL") then
        call reada(aa)
        ! print "(a,i0,2a)", "Rank limited to ", l, " for all ", trim(aa)
        do i=1,ns
          ! print "(a)", name(i)
          if (index(name(i),trim(aa)) == 1) then
            ! print "(a, i0)", "Limited to rank ", l
            limit(i)=l
          end if
        end do
      else
...
!  Construct the multipole moment contributions from the current overlap density
...
XY2 = GX(2)*GY(0) + GX(0)*GY(2)
Q(2) = Q(2) + F*(XY0*GZ(2) - 0.5D0*XY2*GZ(0))
if (L == 2) return

Q(3) = Q(3) + F*(XY0*GZ(3) - 1.5D0*XY2*GZ(1))
if (L == 3) return

XY4 = GX(4)*GY(0) + 2.0D0*GX(2)*GY(2) + GX(0)*GY(4)
Q(4) = Q(4) + 0.125D0*F*                                                &
          (8.0D0*XY0*GZ(4) - 24.0D0*XY2*GZ(2) + 3.0D0*XY4*GZ(0))
if (L == 4) return

Q(5) = Q(5) + 0.125D0*F*                                                &
          (8.0D0*GZ(5)*XY0 - 40.0D0*GZ(3)*XY2 + 15.0D0*GZ(1)*XY4)
if (L == 5) return

XY6 = GX(6)*GY(0) + 3.0D0*GX(4)*GY(2) + 3.0D0*GX(2)*GY(4) + GX(0)*GY(6)
Q(6) = Q(6) + 0.0625D0*F*                                               &
          (16.0D0*XY0*GZ(6) - 120.0D0*XY2*GZ(4) + 90.0D0*XY4*GZ(2)      &
            - 5.0D0*XY6*GZ(0))
if (L == 6) return

Q(7) = Q(7) + 0.0625D0*F*                                               &
          (16.0D0*XY0*GZ(7) - 168.0D0*XY2*GZ(5) + 210.0D0*XY4*GZ(3)     &
           - 35.0D0*XY6*GZ(1))
if (L == 7) return

XY8 = GX(8)*GY(0) + 4.0D0*GX(6)*GY(2) + 6.0D0*GX(4)*GY(4) +             &
    4.0D0*GX(2)*GY(6) + GX(0)*GY(8)
Q(8) = Q(8) + 0.0078125D0*F*                                            &
          (128.0D0*XY0*GZ(8) - 1792.0D0*XY2*GZ(6) + 3360.0D0*XY4*GZ(4)  &
           - 1120.0D0*XY6*GZ(2) + 35.0D0*XY8*GZ(0))
if (L == 8) return

Q(9) = Q(9) + 0.0078125D0*F*                                            &
          (128.0D0*XY0*GZ(9) - 2304.0D0*XY2*GZ(7) + 6048.0D0*XY4*GZ(5)  &
           - 3360.0D0*XY6*GZ(3) + 315.0D0*XY8*GZ(1))
if (L == 9) return

XY10 = GX(10)*GY(0) + 5.0D0*GX(8)*GY(2) + 10.0D0*GX(6)*GY(4) +          &
    10.0D0*GX(4)*GY(6) + 5.0D0*GX(2)*GY(8) + GX(0)*GY(10)
Q(10) = Q(10) + 0.00390625D0*F*                                         &
          (256.0D0*XY0*GZ(10) - 5760.0D0*XY2*GZ(8) + 20160.0D0*XY4*GZ(6) &
           - 16800.0D0*XY6*GZ(4) + 3150.0D0*XY8*GZ(2) - 63.0D0*XY10*GZ(0))
if (L == 10) return

Q(11) = Q(11) + 0.00390625D0*F*                                         &
          (256.0D0*XY0*GZ(11) - 7040.0D0*XY2*GZ(9) + 31680.0D0*XY4*GZ(7) &
           - 36960.0D0*XY6*GZ(5) + 11550.0D0*XY8*GZ(3) - 693.0D0*XY10*GZ(1))
return
...
e1=exp(-aa*z1**2)/(2d0*aa)
e2=exp(-aa*z2**2)/(2d0*aa)
gz(1,0,0)=e1-e2
...
INTEGER :: i, i1, i2, ia, ib, ii, ii1, ii2, ig, iq,                    &
    ...
    l, la, lb, loci, locj, lq, m, mi, mj, mini, maxi, minj, maxj, nq
...
!  Monopole term
Q00 = Q00 + f*gx(0)*gy(0)*gz(0)
if (LL == 0) return

!  Dipole terms
Q10 = Q10 + f*(gx(0)*gy(0)*gz(1))
